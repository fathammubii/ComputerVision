...
def detection():
    ...
    global img1, iago, img2, iago2, blmaskRez, iago3, lab
    ...
    lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB) #konversi color
    
    black_low = np.array([0, 140, 0], np.uint8)
    black_high = np.array([248, 255, 142], np.uint8)
    black_mask = cv2.inRange(lab, black_low, black_high)
...
def gui():
    ...
    win.title("Deteksi Penyakit Busuk Daun Selada dengan Ruang Warna LAB")
    ...
    judulruang = Label(frame_1,text = 'Ruang Warna LAB', font=("arial",21),bg='#929292').place(x= 856,y = 26, width=300)
...
if __name__ == '__main__':
    ...
    del img1, iago, img2, iago2, blmaskRez, iago3, lab